...
static int existeCodigoZona(const Zone zonas[], int cantidad, const char *codigo) {
...
static void crearNombreReporteUnico(char *dest, size_t tam, const char *archivoBase) {
    const char *ext = strrchr(archivoBase, '.');
    size_t baseLen = (ext != NULL) ? (size_t)(ext - archivoBase) : strlen(archivoBase);
    int contador = 0;

    snprintf(dest, tam, "%s", archivoBase);
    ...
    while (1) {
        ...
        if (ext != NULL) {
            snprintf(dest, tam, "%.*s_%d%s", (int)baseLen, archivoBase, contador, ext);
        } else {
            snprintf(dest, tam, "%s_%d", archivoBase, contador);
        }
    }
}
...
void exportarReporte(const Zone zonas[], int cantidad, const Alerta alertas[], int alertas_count, const Prediction predicciones[], int predicciones_count, const char *archivo) {
    char rutaReporte[200];
    FILE *fp;

    crearNombreReporteUnico(rutaReporte, sizeof(rutaReporte), archivo);
    fp = fopen(rutaReporte, "w");
    if (fp == NULL) {
    ...
    fclose(fp);
    printf("\n[OK] Reporte exportado correctamente a '%s'\n", rutaReporte);
}
...
void agregarZona(Zone zonas[], int *cantidad) {
    ...
    while (1) {
        ...
        if (validarVacio(codigo_temp)) {
            ...
            if (valido) {
                if (existeCodigoZona(zonas, *cantidad, codigo_temp)) {
                    printf("  [!] El codigo ya existe. Ingrese un ID unico para la zona.\n");
                    continue;
                }
    ...
    printf("\nIngrese los datos ambientales iniciales de la zona:\n");
    ingresarRegistroDiario(nueva);

    (*cantidad)++;
    printf("\n[OK] Zona agregada correctamente.\n");
}
...
void ingresarRegistroDiario(Zone *zona) {
    ...
    printf("\n[OK] Registro agregado exitosamente.\n");
}
...
void mostrarMonitoreoActual(const Zone zonas[], int cantidad) {
    ...
    printf("+--------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
    printf("| Zona               | CO2    | SO2    | PM2.5  | PM10   | CO     | NO2    | O3     | Temp   | Viento | Humed. | Calidad       |\n");
    printf("+--------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
    for (int i = 0; i < cantidad; i++) {
        const Zone *zona = &zonas[i];
        ...
        printf("| %-18s | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %-13s |\n",
               zona->nombre, co2, so2, pm25, pm10, co, no2, o3, temperatura, viento, humedad, clasificarCalidad(indice));
    }
    printf("+--------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
}
...
void mostrarPromediosHistoricos(const Zone zonas[], int cantidad) {
    ...
    printf("+--------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+\n");
    printf("| Zona               | CO2    | SO2    | PM2.5  | PM10   | CO     | NO2    | O3     | Temp   | Viento | Humed. |\n");
    printf("+--------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+\n");
    for (int i = 0; i < cantidad; i++) {
        const Zone *zona = &zonas[i];
        float sumaTemperatura = 0.0f;
        float sumaViento = 0.0f;
        float sumaHumedad = 0.0f;

        for (int j = 0; j < zona->registros_count; j++) {
            const DailyRecord *registro = &zona->registros[j];
            sumaTemperatura += registro->temperatura;
            sumaViento += registro->viento;
            sumaHumedad += registro->humedad;
        }

        float promedioTemperatura = zona->registros_count > 0 ? sumaTemperatura / zona->registros_count : 0.0f;
        float promedioViento = zona->registros_count > 0 ? sumaViento / zona->registros_count : 0.0f;
        float promedioHumedad = zona->registros_count > 0 ? sumaHumedad / zona->registros_count : 0.0f;

        printf("| %-18s | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f |\n",
               zona->nombre,
...
static void agregarRecomendacionTexto(char *mensaje, size_t tam, const char *texto) {
    size_t len;

    if (mensaje == NULL || tam == 0 || texto == NULL || texto[0] == '\0') {
        return;
    }
    ...
    if (len > 0) {
        ...
        strncat(mensaje, " | ", tam - len - 1);
        len = strlen(mensaje);
    }

    if (len + strlen(texto) + 1 < tam) {
        strncat(mensaje, texto, tam - len - 1);
    }
}

static void generarRecomendacionesPorParametros(const Zone *zona, const Prediction *pred, char *mensaje, size_t tam) {
    const char *abrev[7] = {"PM2.5", "PM10", "CO", "NO2", "O3", "CO2", "SO2"};
    ...
    for (int i = 0; i < 7; i++) {
        float limite = obtenerLimiteContaminante(abrev[i]);
        if (limite > 0.0f && valores[i] > limite) {
            char detalle[180];
            snprintf(detalle, sizeof(detalle), "%s (%s) alto: %s", obtenerNombreContaminante(abrev[i]), abrev[i], obtenerRecomendacion(zona->tipo, abrev[i]));
            agregarRecomendacionTexto(mensaje, tam, detalle);
        }
    }

    if (pred->temperatura > g_referencia.umbralTemperaturaAlta) {
        agregarRecomendacionTexto(mensaje, tam, "Temperatura alta: reducir la exposicion solar y mantener hidratacion.");
    } else if (pred->temperatura < g_referencia.umbralTemperaturaBaja) {
        agregarRecomendacionTexto(mensaje, tam, "Temperatura baja: protegerse del frio y mantener ambiente cerrado.");
    }

    if (pred->humedad > g_referencia.umbralHumedadAlta) {
        agregarRecomendacionTexto(mensaje, tam, "Humedad alta: ventilar y reducir la humedad en interiores.");
    } else if (pred->humedad < g_referencia.umbralHumedadBaja) {
        agregarRecomendacionTexto(mensaje, tam, "Humedad baja: mantener ambientes humedecidos y beber agua.");
    }

    if (pred->viento < g_referencia.umbralVientoCalma) {
        agregarRecomendacionTexto(mensaje, tam, "Viento muy bajo: monitorear acumulacion de contaminantes.");
    } else if (pred->viento > g_referencia.umbralVientoModerado) {
        agregarRecomendacionTexto(mensaje, tam, "Viento fuerte: reducir actividades al aire libre y reforzar seguridad.");
    }

    if (mensaje[0] == '\0') {
        if (strcmp(zona->tipo, "Industrial") == 0) {
            snprintf(mensaje, tam, "Revisar emisiones industriales y activar filtros.");
        } else if (strcmp(zona->tipo, "Residencial") == 0) {
            snprintf(mensaje, tam, "Evitar actividades al aire libre y mantener ventanas cerradas.");
        } else {
            snprintf(mensaje, tam, "Restringir la circulacion de vehiculos e informar al publico.");
        }
    }
}
...
void generarPrediccion(const Zone *zona, Prediction *prediccion) {
    ...
    generarRecomendacionesPorParametros(zona, prediccion, prediccion->mensaje, sizeof(prediccion->mensaje));
}

static void agregarAlertaParametro(Alerta alertas[], int *alertas_count, const Zone *zona, const char *nombreParametro, const char *abreviatura, float valor, float limite, const char *categoria, const char *recomendacion) {
    ...
    strncpy(alerta->codigoZone, zona->codigo, MAX_CODIGO - 1);
    alerta->codigoZone[MAX_CODIGO - 1] = '\0';
    strncpy(alerta->nombreZona, zona->nombre, MAX_NOMBRE);
    ...
    strncpy(alerta->contaminante, nombreParametro, sizeof(alerta->contaminante) - 1);
    alerta->contaminante[sizeof(alerta->contaminante) - 1] = '\0';
    strncpy(alerta->abreviatura, abreviatura, sizeof(alerta->abreviatura) - 1);
    alerta->abreviatura[sizeof(alerta->abreviatura) - 1] = '\0';
    alerta->valor = valor;
    alerta->limite = limite;
    strncpy(alerta->categoria, categoria, sizeof(alerta->categoria) - 1);
    alerta->categoria[sizeof(alerta->categoria) - 1] = '\0';
    strncpy(alerta->recomendacion, recomendacion, MAX_MENSAJE - 1);
    ...
    *predicciones_count = 0;
    for (int i = 0; i < cantidad; i++) {
        ...
        for (int j = 0; j < 7; j++) {
            ...
            if (limite > 0.0f && valores[j] > limite) {
                char recomendacion[180];
                snprintf(recomendacion, sizeof(recomendacion), "%s", obtenerRecomendacion(zonas[i].tipo, abrev[j]));
                agregarAlertaParametro(alertas, alertas_count, &zonas[i], obtenerNombreContaminante(abrev[j]), abrev[j], valores[j], limite, "Alto", recomendacion);
            }
        }

        if (pred.temperatura > g_referencia.umbralTemperaturaAlta) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Temperatura", "TEMP", pred.temperatura, g_referencia.umbralTemperaturaAlta, "Fuera de rango", "Reducir la exposicion solar y mantener hidratacion.");
        } else if (pred.temperatura < g_referencia.umbralTemperaturaBaja) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Temperatura", "TEMP", pred.temperatura, g_referencia.umbralTemperaturaBaja, "Fuera de rango", "Protegerse del frio y mantener ambiente cerrado.");
        }

        if (pred.humedad > g_referencia.umbralHumedadAlta) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Humedad", "HUM", pred.humedad, g_referencia.umbralHumedadAlta, "Fuera de rango", "Ventilar y reducir la humedad en interiores.");
        } else if (pred.humedad < g_referencia.umbralHumedadBaja) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Humedad", "HUM", pred.humedad, g_referencia.umbralHumedadBaja, "Fuera de rango", "Mantener ambientes humedecidos y beber agua.");
        }

        if (pred.viento < g_referencia.umbralVientoCalma) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Viento", "VIE", pred.viento, g_referencia.umbralVientoCalma, "Fuera de rango", "Monitorear acumulacion de contaminantes.");
        } else if (pred.viento > g_referencia.umbralVientoModerado) {
            agregarAlertaParametro(alertas, alertas_count, &zonas[i], "Viento", "VIE", pred.viento, g_referencia.umbralVientoModerado, "Fuera de rango", "Reducir actividades al aire libre y reforzar seguridad.");
        }
    }
}
...
void mostrarPredicciones(const Zone zonas[], int cantidad, const Prediction predicciones[], int predicciones_count) {
    ...
    printf("+-------------+------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
    printf("| Codigo      | Zona             | CO2    | SO2    | PM2.5  | PM10   | CO     | NO2    | O3     | Temp   | Viento | Nivel         |\n");
    printf("+-------------+------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
    for (int i = 0; i < predicciones_count; i++) {
        const Prediction *pred = &predicciones[i];
        printf("| %-11s | %-16s | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %-13s |\n",
               pred->codigoZona, pred->nombreZona, pred->co2, pred->so2, pred->pm25, pred->pm10, pred->co, pred->no2, pred->o3, pred->temperatura, pred->viento, pred->nivel);
    }
    printf("+-------------+------------------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------------+\n");
}

void mostrarRecomendaciones(const Zone zonas[], int cantidad, const Prediction predicciones[], int predicciones_count) {
    (void)zonas;
    (void)cantidad;
    mostrarEncabezado("RECOMENDACIONES GENERADAS");

    if (predicciones_count == 0) {
        printf("No hay recomendaciones disponibles aun.\n");
        return;
    }

    for (int i = 0; i < predicciones_count; i++) {
        const Prediction *pred = &predicciones[i];
        printf("\n[%s] %s\n", pred->codigoZona, pred->nombreZona);
        printf("  Nivel: %s\n", pred->nivel);
        printf("  %s\n", pred->mensaje[0] != '\0' ? pred->mensaje : "Sin recomendaciones especificas.");
    }
}
...
void mostrarAlertas(const Alerta alertas[], int alertas_count) {
    ...
    printf("+------------+----------+----------------------+--------------+--------+--------+----------------------+\n");
    printf("| Fecha      | Hora     | Zona                 | Contaminante | Valor  | Limite | Estado              |\n");
    printf("+------------+----------+----------------------+--------------+--------+--------+----------------------+\n");
    for (int i = 0; i < alertas_count; i++) {
        printf("| %-10s | %-8s | %-20s | %-12s | %6.2f | %6.2f | %-20s |\n",
...
void mostrarTodasZonasOrdenadas(const Zone zonas[], int cantidad) {
    ...
    printf("+---+------------------+--------+--------+--------+--------+--------+--------+--------------+\n");
    printf("| # | Zona             | CO2    | PM2.5  | PM10   | CO     | NO2    | O3     | Calidad       |\n");
    printf("+---+------------------+--------+--------+--------+--------+--------+--------+--------------+\n");
    for (int i = 0; i < cantidad; i++) {
        const Zone *zona = &zonas[indices[i]];
        const DailyRecord *registro = (zona->registros_count > 0) ? &zona->registros[zona->registros_count - 1] : NULL;
        float indice = calcularIndiceCompuesto(zona);
        printf("| %-2d| %-16s | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %6.2f | %-13s |\n",
               i + 1,
               zona->nombre,
               registro != NULL ? registro->co2 : 0.0f,
               registro != NULL ? registro->pm25 : 0.0f,
               registro != NULL ? registro->pm10 : 0.0f,
               registro != NULL ? registro->co : 0.0f,
               registro != NULL ? registro->no2 : 0.0f,
               registro != NULL ? registro->o3 : 0.0f,
               clasificarCalidad(indice));
    }
    printf("+---+------------------+--------+--------+--------+--------+--------+--------+--------------+\n");
}
...
void modificarZona(Zone zonas[], int cantidad) {
    ...
    printf("\n[OK] Parametro actualizado con exito para la zona %s.\n", zona_a_modificar->nombre);
}
...
void eliminarZona(Zone zonas[], int *cantidad) {
    ...
    printf("\nEsta seguro de eliminar completamente la zona %s? (1: Si / 2: No): ", zonas[seleccion].nombre);
    int confirmacion;
    ...
    if (confirmacion == 1) {
        ...
        (*cantidad)--;
        printf("\n[OK] La zona ha sido removida del sistema exitosamente.\n");
    } else {
        printf("\nOperacion cancelada.\n");
    }
}

static void mostrarSubmenuOpciones(void) {
    printf("\n---------------------------------------------------------\n");
    printf("  SUBMENU - OTRAS OPCIONES\n");
    printf("---------------------------------------------------------\n");
    printf("  1. Ver recomendaciones generadas\n");
    printf("  2. Mostrar alertas preventivas activas\n");
    printf("  3. Consultar informacion especifica por zona\n");
    printf("  4. Visualizar ranking de zonas mas contaminadas\n");
    printf("  5. Revisar estadisticas generales resumidas\n");
    printf("  6. Modificar un solo dato especifico de una zona\n");
    printf("  7. Eliminar/Dar de baja una zona\n");
...
void mostrarMenu(void) {
    printf("\n---------------------------------------------------------\n");
    printf("  MENU PRINCIPAL - SIGPCAZU\n");
    printf("---------------------------------------------------------\n");
    printf("  1. Agregar nueva zona al sistema\n");
    printf("  2. Registrar entrada de lecturas diarias\n");
    ...
    printf("  6. Exportar reporte general (Archivo de texto)\n");
    printf("  7. Otras opciones\n");
    printf("  0. Salir del sistema\n");
    printf("---------------------------------------------------------\n");
    printf("  Seleccione una opcion: ");
}

static void ejecutarSubmenu(Zone zonas[], int *cantidad, Alerta alertas[], int *alertas_count, Prediction predicciones[], int *predicciones_count) {
    int opcion;
    while (1) {
        mostrarSubmenuOpciones();
        leerEntero(&opcion, 0, 7, "");

        switch (opcion) {
            case 1:
                generarAlertas(zonas, *cantidad, alertas, alertas_count, predicciones, predicciones_count);
                mostrarRecomendaciones(zonas, *cantidad, predicciones, *predicciones_count);
                break;
            case 2:
                generarAlertas(zonas, *cantidad, alertas, alertas_count, predicciones, predicciones_count);
            ...
            case 7:
                ...
                break;
            case 0:
                printf("\n[OK] Regresando al menu principal.\n");
                return;
        }
    }
}
    ...
    while (1) {
        ...
        switch (opcion) {
            ...
            case 6:
                ...
                break;
            case 7:
                ejecutarSubmenu(zonas, cantidad, alertas, alertas_count, predicciones, predicciones_count);
                break;
            case 0:
